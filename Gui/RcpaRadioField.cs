...
namespace RCPA.Gui
{
  public partial class RcpaRadioField : RadioButton, IRcpaComponent
  {
    public RcpaRadioField()
    {
      InitializeComponent();
    ...
    protected virtual void DoPreConditionChanged(object sender, EventArgs e)
    {
      if (sender is CheckBox)
      {
        ...
        this.Enabled = cb.Enabled && cb.Checked;
      }
    }
    ...
    {
      new RcpaRadioButton(this, Key, Checked).RemoveFromXml(option);
    }

    public void LoadFromXml(System.Xml.Linq.XElement option)
    {
      new RcpaRadioButton(this, Key, Checked).LoadFromXml(option);
    }

    public void SaveToXml(System.Xml.Linq.XElement option)
    {
      new RcpaRadioButton(this, Key, Checked).SaveToXml(option);
    }
